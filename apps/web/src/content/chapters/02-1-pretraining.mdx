...
```mermaid
graph TD
  A["Input Context: 'Once upon a ___'"] --> B["Embedding & Attention Layers (Weights W)"]
  B --> C["Model Logit Output (Probabilities)"]
  C --> D{"Compute Cross-Entropy Loss: L = -ln(P(target))"}
  D -->|Loss > 0 (Low confidence)| E["Backpropagation: Compute Gradients"]
  E --> F["Adjust Weights: W_new = W - &eta; * Gradient"]
  F --> A
  D -->|Loss &approx; 0 (High confidence)| G["Continue to Next Token"]
```
