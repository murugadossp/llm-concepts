...
```mermaid
graph TD
  User["User Query: 'What were yesterday's sales?'"] --> Embed["Query Embedding Generator"]
  Embed -->|Vector Coordinates| VDB[("Vector Database\n(Pgvector, Pinecone)")]
  VDB -->|Retrieve Top K Chunks| Ranker["Re-ranker / Context Compiler"]
  Ranker -->|Context: 'Yesterday sales: $42,000'| PromptBuilder["Augmented Prompt Assembler"]
  User --> PromptBuilder
  PromptBuilder -->|Augmented Prompt| LLM["Large Language Model"]
  LLM -->|Generates Answer| Answer["User Output: 'Yesterday's sales were $42,000'"]
```
