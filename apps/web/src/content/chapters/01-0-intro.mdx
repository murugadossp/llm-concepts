...
```mermaid
graph TD
  Prompt["Input Prompt: 'The sky is'"] --> LLM["LLM (Text Predictor)"]
  LLM -->|Outputs Probability Pool| Pool["'blue' (85%), 'grey' (10%), 'cloudy' (3%)"]
  Pool -->|Select 'blue'| Select["Chosen Token: 'blue'"]
  Select -->|Append to Prompt| Append["New Sequence: 'The sky is blue'"]
