...
```mermaid
graph TD
  Query["User Query"] --> BM25["Lexical Search<br/>(BM25)"] & Vec["Semantic Search<br/>(Vector Embeddings)"]
  BM25 -->|Top 50 Chunks| Merge["RRF Rank Fusion<br/>Merger"]
  Vec -->|Top 50 Chunks| Merge
  Merge -->|Top 50 Candidates| Cross["Cross-Encoder /<br/>Re-Ranker"]
  Cross -->|Concatenated Match| Sort["Sorted Top 5<br/>Best Chunks"]
  Sort --> Prompt["Inject into<br/>Prompt Context"]
```
