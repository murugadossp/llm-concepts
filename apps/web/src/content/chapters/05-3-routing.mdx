...
```mermaid
graph TD
  Prompt["User Prompt:<br/>'Book a room for Tuesday'"] --> Embed["Router Embedding<br/>Model"]
  Embed -->|Query Vector| Search["Vector DB<br/>Similarity Search"]
  
  subgraph Tool Catalog Database
    T1["Tool Schema:<br/>room_booking"]
    T2["Tool Schema:<br/>calendar_create"]
    T3["Tool Schema:<br/>fetch_weather"]
    T4["Tool Schema:<br/>database_query"]
  end

  Search -->|Nearest Neighbors| T1 & T2
  T1 & T2 -->|Dynamic Schema Injection| APIRequest["LLM API Request<br/>Context"]
  Prompt --> APIRequest
  APIRequest -->|Only 2 Schemas passed| LLM["LLM (Active Model)"]
  LLM -->|Emits ToolCall| Execute["JSON:<br/>room_booking(...)"]
```
