...
- Hero panel: stylized "From a single token to an army of agents" headline on a solid theme-aware surface over a very soft gradient mesh, with a restyled SVG of the learning journey (Tess the Token → Atlas the Agent).
- "Learning path" visualization: a clean solid roadmap with 11 numbered milestones, each clickable, with checkmark badges for chapters the signed-in user has completed.
- Chapter grid: 11 solid cards. Each card has a restyled character icon, title, one-line teaser, estimated read time, difficulty badge (Beginner / Intermediate / Advanced), and a tier chip (Free / Pro).
...
1. **Left learning sidebar** — collapsible on desktop, drawer on mobile, with chapter → lesson nesting and current lesson highlight.
2. **Sticky top nav** — back to hub, prev/next lesson, compact chapter/lesson label.
3. **Chapter / lesson hero** — chapter number, optional lesson number, title, one-sentence promise, hand-drawn SVG hero illustration.
4. **TL;DR card** — 3 bullets, what you'll learn.
5. **Comic strip** — 2–4 panels introducing the core concept via analogy.
6. **Concept sections** — each concept has: heading, ELI5 speech bubble, core explanation, infographic/diagram, deep-dive `<details>`.
7. **Comparison table** — when the lesson introduces multiple related concepts.
8. **Remember card** — visual cheat-sheet of the lesson's takeaways.
9. **Try-it prompt** — a small interactive widget OR a "try this prompt yourself" callout.
10. **Bottom nav** — prev/next lesson, return to hub.

---

## 4. Visual design system

The site uses a **solid learning-app aesthetic** — opaque theme-aware surfaces, restrained borders, soft shadows, ample whitespace, and a subdued gradient mesh background — in both **light** and **dark** themes, toggleable per user, with system-preference detection on first load.

The recurring cast of characters (§4.4) is preserved as a memorability asset — they're restyled with cleaner vector lines, soft gradients, and a theme-aware palette so they sit naturally on solid surfaces in either theme.
...
| `--surface-strong` | `#FFFFFF` | Raised card / app-shell surface |
| `--surface-muted` | `#F7F8FC` | Quiet nested surface |
| `--surface-hover` | `#EEF1FB` | Hover / selected background |
| `--border` | `rgba(15,18,38,0.08)` | Subtle border (1px) |
...
| `--surface` | `#111326` | Primary card / panel surface |
| `--surface-strong` | `#16192D` | Raised card / app-shell surface |
| `--surface-muted` | `#1D2138` | Quiet nested surface |
| `--surface-hover` | `#252A45` | Hover / selected background |
...
- Background mesh: intentionally low opacity; content surfaces are opaque.
...
|-----------|----------------------|
| `ComicPanel` | Solid card with subtle border, restyled character SVG inside, soft drop-shadow |
| `ComicStrip` | Row of 2–4 panels; horizontal snap-scroll on mobile |
| `SpeechBubble` | Solid bubble with tail, hand-drawn dialogue font |
| `ELI5Card` | Solid card with violet accent + 🧒 chip |
| `DeepDive` | `<details>` styled as a collapsible solid panel; accent border when open |
| `Infographic` | Centered SVG, solid background, captions below |
| `CompareTable` | Solid surface, zebra rows via quiet theme tokens, sticky header |
| `RememberCard` | Solid card with sun-yellow accent + 💡 chip |
| `FlowDiagram` | SVG with translucent connectors, glowing nodes |
| `CodeBlock` | Semantic dark code surface in both themes; JetBrains Mono; copy button |
| `CodeTabs` | Tabbed TS / Python variants |
| `Chip` | Pill, solid/tinted background, color by category |
| `Callout` | Solid left-border accent; variants `warn` / `tip` / `myth` |
| `PersonaStrip` | Row of small character avatars on a solid bar |
| `Paywall` | Premium solid gradient card with upgrade CTA (Pro tier) |
| `ThemeToggle` | Header pill with light / dark / system options |
| `ChapterSidebar` | Solid app-style sidebar; light in light theme, dark in dark theme; desktop collapse and mobile drawer |

### 4.4 Recurring characters (restyled for the solid learning UI)
...
|-----------|------|---------------|
| **Tess the Token** | Personifies a single token | Rounded square with friendly face, gradient fill, holding a text snippet |
| **Vector the Embedding** | Personifies vector space | Arrow-bodied character with gradient tail, pointing across N dimensions |
| **Atlas the Agent** | Personifies an agent | Hero-sized, theme-aware tool-belt with 🔨 hammer + 🔍 magnifier |
| **MCP-Mae** | Personifies the MCP server | Switchboard operator with glowing patch cables, theme-aware headset |
...
Chapter 1 is implemented as a **module with sub-lessons** rather than one crowded long page. The overview page introduces the map; individual lessons teach one idea at a time:
...
- **Acronyms:** expanded on first use *per chapter / lesson*; wrapped in `<abbr>` with hover definition.
...
- Solid learning-app design system (light + dark + system)
...
| Visual style | **Solid learning-app UI** with light + dark themes, system-preference detection, opaque surfaces, subtle borders, low-opacity mesh, and a collapsible learning sidebar. Comic characters remain theme-aware; cast unchanged. | 2026-05-23 |
| Chapter structure | **Chapters may contain sub-lessons.** Chapter 1 is split into an overview plus lessons 1.1–1.5 to reduce cognitive load. | 2026-05-23 |
